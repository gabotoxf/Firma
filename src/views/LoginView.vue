...
const handleLogin = async () => {
  ...
  try {
    ...
    if (credentials.value.email === 'admin@erabogados.com' && credentials.value.password === 'admin123') {
      localStorage.setItem('auth_token', 'demo-token-001')
      localStorage.setItem('user_name', 'Juan')
      localStorage.setItem('user_role', 'admin')
      localStorage.setItem('user_image', '/img/socios/gustavo.jfif')
    ...
    <div class="hidden lg:flex w-[45%] flex-col justify-between bg-primary p-14 relative overflow-hidden">
      ...
      <div class="relative z-10">
        ...
        <p class="font-headline text-[18px] text-white/80 leading-relaxed font-light mb-3">
          Gestión de
          <span class="text-secondary font-semibold">contenidos</span>
          editoriales
        </p>
        <p class="font-label text-[11px] text-white/30 tracking-[0.15em] uppercase">Sistema interno</p>
      </div>
    ...
    <div class="flex-1 flex items-center justify-center px-8 py-12">
      <div class="w-full max-w-[340px]">
        ...
        <div class="mb-10">
          ...
          <p class="font-label text-[12px] text-gray-400 tracking-wide">CMS · ER Abogados</p>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="flex flex-col gap-6">
          <div>
            ...
            <input v-model="credentials.email" type="email" required placeholder="admin@erabogados.com"
